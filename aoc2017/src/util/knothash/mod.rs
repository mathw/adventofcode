...
pub fn knot_hash<I>(input: I) -> String
where
...
pub fn hash<T>(list: &[T], lengths: &[usize]) -> T
where
    T: Copy + Mul<Output = T>,
{
...
mod tests {
    ...
    mod knot_hash {
        use super::super::knot_hash;

        #[test]
        fn sample_one() {
            assert_eq!(
                knot_hash("".chars().map(|c| c as u8)),
        ...
        fn sample_two() {
            assert_eq!(
                knot_hash("AoC 2017".chars().map(|c| c as u8)),
        ...
        fn sample_three() {
            assert_eq!(
                knot_hash("1,2,3".chars().map(|c| c as u8)),
        ...
        fn sample_four() {
            assert_eq!(
                knot_hash("1,2,4".chars().map(|c| c as u8)),
